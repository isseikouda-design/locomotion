...
/* =========================================================
   SP UI wiring
========================================================= */
function buildSpStrip() {
  const strip = document.getElementById('spStrip');
  if (!strip) return;

  strip.innerHTML = '';

  const scenes = MODELS.filter((m) => /^scene\d{3}$/.test(m.id)).sort((a, b) => a.id.localeCompare(b.id));
  const objects = MODELS
    .filter((m) => /^object\d{3}$/.test(m.id) || m.id === 'objectXXX')
    .sort((a, b) => {
      if (a.id === 'objectXXX') return 1;
      if (b.id === 'objectXXX') return -1;
  ...
  scenes.forEach((m) => strip.appendChild(makeBtn(m.id)));

  const sep = document.createElement('span');
  sep.className = 'sp-strip__sep';
...
function ensureSpObjectPanel() {
  ...
  let panel = bottom.querySelector('.sp-object-panel');
  if (!panel) {
    panel = document.createElement('div');
    panel.className = 'sp-object-panel';
    panel.innerHTML = `
      <div class="sp-object-row sp-object-head"></div>
      <div class="sp-object-row sp-object-desc"></div>
      <div class="sp-object-row sp-object-buy">
        <a class="sp-obj-buylink" href="#" rel="noopener">buy/ask</a>
      </div>
    `;
...
(function wireSpUi() {
  ...
  const strip = document.getElementById('spStrip');
  const gotoBtn = document.getElementById('spGotoBtn');
  const starBtn = document.getElementById('spStarBtn');
  const infoBtn = document.getElementById('spInfoBtn');
  ...
  if (strip) {
    strip.addEventListener(
      ...
      (e) => {
        ...
        if (id.startsWith('scene')) u.searchParams.set('scene', id.replace('scene', ''));
        if (id.startsWith('object')) u.searchParams.set('object', id.replace('object', ''));

        history.pushState({ type: id.startsWith('scene') ? 'scene' : 'object', id }, '', u);
      },
      { passive: false }
    );
  }

function syncSpStarUi() {
  const btn = document.getElementById('spStarBtn');
  if (!btn) return;
  ...
  const label = btn.querySelector('.sp-star__label');
  if (label) label.textContent = isOn ? 'stop' : 'star';

  btn.setAttribute('aria-label', isOn ? 'close star trail' : 'star');
}
...
if (starBtn) {
  starBtn.addEventListener(
    ...
    (e) => {
      ...
      if (locus) {
        ...
        document.body.classList.toggle('is-trail');
  ...
  console.warn('[SP UI] #spStarBtn not found');
}
  ...
  infoBtn.addEventListener(
    ...
    (e) => {
      ...
      if (isSP) {
        ...
        const isOpen = document.body.classList.contains('sp-info-open');

        // すでに開いているなら、共通の closeInfo を使う
        if (isOpen) {
          if (typeof window.closeSpInfo === 'function') {
  window.closeSpInfo();
}else {
            document.body.classList.remove('sp-info-open');
        ...
        // 開く
        document.body.classList.add('sp-info-open');
      ...
      // PC：従来どおりoverlay
      window.openOverlayPanel?.('about');
    },
    { passive: false }
  );
}

  window.__spUiUpdate = function (item) {
    if (!isMobile()) return;
    if (!item?.id) return;

    document.body.classList.toggle('is-sp-object', item.id.startsWith('object'));

    const bottom = document.querySelector('.sp-bottom');
    const status = bottom?.querySelector('.sp-bottom__status');
    const idLine = status?.querySelector('.sp-scene-id');

    if (idLine) idLine.textContent = item.id;

    const isObj = item.id.startsWith('object');
    if (status) status.style.display = isObj ? 'none' : '';

    // goto更新（sceneのとき）
    if (gotoBtn && item.id.startsWith('scene')) {
      const obj = item.goObjectId || `object${item.id.replace('scene', '')}`;
      gotoBtn.textContent = `→ Go to ${obj}`;
      gotoBtn.setAttribute('data-goto', obj);
    }

    // objectのときだけ3段を出す
    const panel = ensureSpObjectPanel();
    if (panel) {
      panel.style.display = isObj ? 'block' : 'none';

      if (isObj) {
        const head = panel.querySelector('.sp-object-head');
        const desc = panel.querySelector('.sp-object-desc');
        const link = panel.querySelector('.sp-obj-buylink');

        if (head) head.textContent = item.id;

        const info = item.info || null;
        const lines = (info?.lines || []).map((v) => String(v));
        const descLines = lines.filter((t) => !t.toLowerCase().includes('buy/ask'));

        if (desc) {
          desc.innerHTML = descLines.length
            ? descLines.map((t) => `<p class="sp-obj-descline">${t}</p>`).join('')
            : `<p class="sp-obj-descline">Details coming soon.</p>`;
        }

        const addr = info?.email || 'info@locomotion.com';
...
(function () {
  ...
  window.addEventListener('DOMContentLoaded', wireSpGotoButton);
})();
...
/* =========================================================
   SP: 高さ計測 → CSS変数へ
========================================================= */
function updateSpStageVars() {
  const isSP = window.matchMedia('(max-width: 768px)').matches;
  if (!isSP) return;

  const root = document.documentElement;
  const spUI = document.querySelector('.sp-ui');
  const statusEL = document.querySelector('.sp-bottom__status');
  const actionsEL = document.querySelector('.sp-bottom__actions');

  if (spUI) {
    const h = Math.ceil(spUI.getBoundingClientRect().height);
    root.style.setProperty('--sp-ui-h', `${h}px`);
  }

  let statusH = 0;
  if (statusEL && statusEL.offsetParent !== null) {
    statusH = Math.ceil(statusEL.getBoundingClientRect().height);
  }
  root.style.setProperty('--sp-status-h', `${statusH}px`);

  let actionsH = 0;
  if (actionsEL) {
    actionsH = Math.ceil(actionsEL.getBoundingClientRect().height);
  }
  root.style.setProperty('--sp-actions-h', `${actionsH}px`);

  const isObj = document.body.classList.contains('is-sp-object');

  let bottomH = 0;
  if (isObj) {
    bottomH += actionsH;
    const panel = document.querySelector('.sp-object-panel');
    if (panel) bottomH += Math.ceil(panel.getBoundingClientRect().height);
  } else {
    bottomH += statusH + actionsH;
  }

  root.style.setProperty('--sp-bottom-h', `${bottomH}px`);
}
function alignObjectModelToSpViewportCenter() {
  const isSP = window.matchMedia('(max-width: 768px)').matches;
  if (!isSP) return;
  if (!document.body.classList.contains('is-sp-object')) return;
  if (!currentModel) return;

  const strip = document.querySelector('.sp-strip');
  const head = document.querySelector('.sp-object-head');
  const canvas = renderer.domElement;

  if (!strip || !head || !canvas) return;
  ...
  const desiredCenterPx = (stripRect.bottom + headRect.top) / 2;
  const canvasCenterPx = canvasRect.top + canvasRect.height / 2;
  const diffPx = desiredCenterPx - canvasCenterPx;

  const worldPerPixel = (camera.top - camera.bottom) / camera.zoom / canvasRect.height;
  const worldShift = diffPx * worldPerPixel;
  ...
  // cameraはYだけ補正
  camera.position.y = spBaseCamY + worldShift;

  // targetはX/Zも毎回モデル中心へ戻す
  controls.target.set(
    center.x,
    center.y + worldShift,
    center.z
...
function syncSpLayoutThenAlign() {
  const isSP = window.matchMedia('(max-width: 768px)').matches;
  if (!isSP) return;

  // ===== scene =====
  if (!document.body.classList.contains('is-sp-object')) {
  updateSpStageVars();
  ...
  requestAnimationFrame(() => {
    onResize();
    refitCurrentSpObjectAfterLayout();

    requestAnimationFrame(() => {
      if (document.body.classList.contains('is-sp-object') && currentModel) {
        currentModel.updateMatrixWorld(true);

        const box = new THREE.Box3().setFromObject(currentModel);
        const sphere = box.getBoundingSphere(new THREE.Sphere());

        spBaseCamY = camera.position.y;
        spBaseTargetY = sphere.center.y;
      }

      alignObjectModelToSpViewportCenter();
      spAlignPending = false;
    });
  });
}
...
/* =========================================================
   SP Info: close buttons hook
========================================================= */
(function wireSpInfoCloseButtons(){
  const closeX  = document.getElementById('spInfoCloseX');
  const closeB  = document.getElementById('spInfoCloseBottom');
  ...
  function closeInfo(){
    ...
    // 3) bottom UI をいったん強制復帰
    if (bottom) {
      bottom.style.display = 'flex';
    }

    // 4) 高さ系CSS変数をいったんクリア
    root.style.removeProperty('--sp-status-h');
    root.style.removeProperty('--sp-actions-h');
    root.style.removeProperty('--sp-bottom-h');

    // 5) 2フレーム待ってから完全再計測
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (typeof updateSpStageVars === 'function') {
          updateSpStageVars();
        }
        ...
        if (typeof alignObjectModelToSpViewportCenter === 'function') {
          alignObjectModelToSpViewportCenter();
        }
      });
    });
